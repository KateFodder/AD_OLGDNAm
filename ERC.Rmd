...
ERC_samples <- subset(targets, Region =="ERC")
rownames(ERC_samples) <- ERC_samples$Sample_Name.x
names(ERC_samples)[2] <- paste("Sample_Name")
ERC_samplenames <- ERC_samples$Sample_Name

myNorm_ERC <- myNorm[,ERC_samplenames]
dim(myNorm_ERC)
```

```{r}
M_matrix_ERC <- beta2m(myNorm_ERC)
```

```{r}
ERC_samples$Slide <- as.character(ERC_samples$Slide)
ERC_samples$Plate <- as.factor(ERC_samples$Plate)

```

```{r}
champ.SVD(beta=as.data.frame(myNorm_ERC), pd = ERC_samples)
```

```{r}
champ.SVD(beta=as.data.frame(M_matrix_ERC), pd = ERC_samples)
```

```{r Removing age associated probes}
ERC_samples <- ERC_samples %>% mutate(Plate = str_replace_all(Plate, " ", "_"))
ERC_samples0 <- ERC_samples[,-c(1,3,14,15,16)]
ERC_samples <- ERC_samples0
Sample_group <- factor(ERC_samples$Sample_Group, levels=c("Control","Alzheimer"))

age <- as.numeric(ERC_samples$Age)

design_ERCage <- model.matrix(~age, data=ERC_samples)
dim(design_ERCage)

fit_ERCage = lmFit(M_matrix_ERC,design_ERCage)

contMatrix_age <-  makeContrasts("age", levels = design_ERCage)

fit2_age <- contrasts.fit(fit_ERCage, contMatrix_age)
...
annSub <- annot[match(rownames(M_matrix_ERC),rownames(probe.genes)),]
...
M_matrix_ERC_t <- as.data.frame(t(M_matrix_ERC))
M_matrix_ERC_t = M_matrix_ERC_t[, !names(M_matrix_ERC_t) %in% age_probes]

M_matrix_ERC_age <- as.matrix(t(M_matrix_ERC_t))
...
```{r}
champ.SVD(beta=as.data.frame(M_matrix_ERC_age), pd = ERC_samples)
```

```{r}
Sample_group <- as.factor(ERC_samples$Sample_Group)

#factors to take into account

Sample_group <- as.factor(ERC_samples$Sample_Group)
sex <- factor(ERC_samples$Sex)
age <- as.numeric(ERC_samples$Age)
array <- factor(ERC_samples$Array)
slide <- as.factor(ERC_samples$Slide)
plate <- as.factor(ERC_samples$Plate)
DoubleN <- as.numeric(ERC_samples$DoubleN)
NeuNP <- as.numeric(ERC_samples$NeuNP)
Sox10P <- as.numeric(ERC_samples$Sox10P)
agesq <- as.numeric((ERC_samples$Age)^2)
#Design Matrix 

design_ERC <- model.matrix(~0+Sample_group+slide+DoubleN+Sox10P+age+agesq, data=ERC_samples0)
dim(design_ERC)



#SVA 
design0 <-  model.matrix(~0+slide+DoubleN+Sox10P+age+agesq, data=ERC_samples0)
n.sv = num.sv(M_matrix_ERC_age, design_ERC, method="leek") ##returns 2 Surrogate Variables 
n.sv
svobj <- sva(M_matrix_ERC_age, design_ERC, design0, n.sv=n.sv)

designSv <- cbind(design_ERC, svobj$sv)

#Fit 

fit = lmFit(M_matrix_ERC_age, design_ERC)

contMatrix <- makeContrasts(Sample_groupAlzheimer-Sample_groupControl, levels = design_ERC)
...
#fit_ERC = lmFit(M_matrix_ERC, design_ERC)


#contMatrix_ERC <- makeContrasts("Sample_groupAlzheimer-Sample_groupControl", levels = design_ERC)

#Checking the fit with SVD 

mAdj.fit_ERC    <- fit$coefficients[,-c(1:2)]

mAdj_ERC        <- as.matrix(M_matrix_ERC_age) - mAdj.fit_ERC %*% t(design_ERC[,-c(1:2)])

bAdj_ERC <- m2beta(mAdj_ERC)


rownames(ERC_samples0) <- ERC_samples0$Sample_Name


```

```{r}
champ.SVD(beta=as.data.frame(bAdj_ERC) , pd =ERC_samples0)
```

```{r}
champ.SVD(beta=as.data.frame(mAdj_ERC) , pd =ERC_samples0)
```


```{r}
fit2_ERC <- contrasts.fit(fit, contMatrix)
fit3_ERC <- eBayes(fit2_ERC)

summary(decideTests(fit3_ERC, adjust.method = "fdr", p.value = 0.05))
```
...
ann450kSub_ERC <- ann450k[match(rownames(M_matrix_ERC),ann450k$Name),
                      c(1:4,12:19,24:ncol(ann450k))]

DMPs_ERC <- topTable(fit3_ERC, num=Inf, coef=1, genelist=ann450kSub_ERC)
```
...
datExpr0  = as.data.frame(t(data3 [,-70])); #remove variance column
...
datTrait1 <- as.data.frame(ERC_samples)
datTrait4 <-datTrait1
...
bwnet = blockwiseModules(datExpr, maxBlockSize = 20000,
                         power = 12, networkType = "signed", minModuleSize = 200,
...
moduleTraitCorK = cor(MEsK, datTraits, use = "p");
...
geneInfo_Hubs <- subset(geneInfo_ERC, Probes%in%Hubs)
...
OLG_probesERC <- intersect(OLG_probes,geneInfo_ERC$Probes)
OPC_probesERC <- intersect(OPC_probes, geneInfo_ERC$Probes)

 # Fisher's test checking for enrichment of Oligodendrocyte genes 

modules <- unique(geneInfo_ERC$moduleColor)
test_results_list <- list()


for(i in modules[1:23]){
  print(i)
  module_ERC <- (subset(geneInfo_ERC, moduleColor == i))
  module_probes <- module_ERC[,1]

 #Getting the numbers 
x = length(intersect(OPC_probesERC, module_probes))
y = (length(OPC_probesERC)- length(intersect(OPC_probesERC, module_probes)))
z =  (length(module_probes)-length(intersect(OPC_probesERC, module_probes)))
p = 45290 - (x+y+z)
...
allMods_ERCmanualOPC <- as.data.frame(do.call(rbind, test_results_list))
...
for(i in modules[1:23]){
 ...
 #Getting the numbers 
x = length(intersect(OLG_probesERC, module_probes))
y = (length(OLG_probesERC)- length(intersect(OLG_probesERC, module_probes)))
z =  (length(module_probes)-length(intersect(OLG_probesERC, module_probes)))
p =45290 - (x+y+z)
...
allMods_ERCmanualOLG <- as.data.frame(do.call(rbind, test_results_list))
```

```{r Forest Plot ERC significant modules}

ERC_OLG_Test <- subset(allMods_ERCmanualOLG, test_module%in%sigmods)
ERC_OLG_Test$test_p_val <- as.numeric(ERC_OLG_Test$test_p_val)
ERC_OLG_Test$test_conflower <- as.numeric(ERC_OLG_Test$test_conflower)
ERC_OLG_Test$test_confupper <- as.numeric(ERC_OLG_Test$test_confupper)
ERC_OLG_Test$test_or <- as.numeric(ERC_OLG_Test$test_or)


colors_ERC<- ERC_OLG_Test$test_module
ERC_OLG_Test$Significant <- paste("Not")
for(i in 1:nrow(ERC_OLG_Test)){
  print(i)
  if(ERC_OLG_Test$test_p_val[i] <0.05){
    ERC_OLG_Test$Significant[i] <- paste("Significant")
  }
}

ERC_OLG_Enrichment_sig <- ggplot(data = ERC_OLG_Test, aes(y=reorder(test_module, test_or), x= test_or, xmin=test_conflower, xmax=test_confupper)) + geom_pointrange(aes(fill=test_module, shape = Significant), size=1.6, show.legend = FALSE) + scale_colour_manual(values = ERC_OLG_Test$test_module) + xlab("Odds Ratio (95% CI)") + ylab("Modules")+ scale_fill_manual(values = ERC_OLG_Test$test_module)+
  theme_bw() + 
  geom_vline(xintercept = 1, linetype="dashed", 
                color = "black") + ggtitle("OLG Enrichment") + scale_shape_manual(values=c(21,22))


ERC_OPC_Test <- subset(allMods_ERCmanualOPC, test_module%in%sigmods)
ERC_OPC_Test$test_p_val <- as.numeric(ERC_OPC_Test$test_p_val)
ERC_OPC_Test$test_conflower <- as.numeric(ERC_OPC_Test$test_conflower)
ERC_OPC_Test$test_confupper <- as.numeric(ERC_OPC_Test$test_confupper)
ERC_OPC_Test$test_or <- as.numeric(ERC_OPC_Test$test_or)



colors_ERC<- ERC_OPC_Test$test_module
ERC_OPC_Test$Significant <- paste("Not")
for(i in 1:nrow(ERC_OPC_Test)){
  print(i)
  if(ERC_OPC_Test$test_p_val[i] <0.05){
    ERC_OPC_Test$Significant[i] <- paste("Significant")
  }
}

ERC_OPC_Enrichment_sig <- ggplot(data = ERC_OPC_Test, aes(y=reorder(test_module, test_or), x= test_or, xmin=test_conflower, xmax=test_confupper)) + geom_pointrange(aes(fill=test_module, shape = Significant), size=1.6, show.legend = FALSE) + scale_colour_manual(values = ERC_OPC_Test$test_module) + xlab("Odds Ratio (95% CI)") + ylab("Modules")+ scale_fill_manual(values = ERC_OPC_Test$test_module)+
  theme_bw() + 
  geom_vline(xintercept = 1, linetype="dashed", 
...
cowplot::plot_grid(ERC_OPC_Enrichment_sig, ERC_OLG_Enrichment_sig, ncol=2, labels=c("OPC Enrichment in FTLD associated Modules", "OLG Enrichment in FTLD associated Modules"))


#~~~ all modules 
ERC_OLG_Test <- allMods_ERCmanualOLG
ERC_OLG_Test$test_p_val <- as.numeric(ERC_OLG_Test$test_p_val)
ERC_OLG_Test$test_conflower <- as.numeric(ERC_OLG_Test$test_conflower)
ERC_OLG_Test$test_confupper <- as.numeric(ERC_OLG_Test$test_confupper)
ERC_OLG_Test$test_or <- as.numeric(ERC_OLG_Test$test_or)
...
ERC_OLG_Enrichment <- ggplot(data = ERC_OLG_Test, aes(y=reorder(test_module, test_or), x= test_or, xmin=test_conflower, xmax=test_confupper)) + geom_pointrange(aes(fill=test_module, shape = Significant), size=1.6, show.legend = FALSE) + scale_colour_manual(values = ERC_OLG_Test$test_module) + xlab("Odds Ratio (95% CI)") + ylab("Modules")+ scale_fill_manual(values = ERC_OLG_Test$test_module)+
  theme_bw() + 
  geom_vline(xintercept = 1, linetype="dashed", 
                color = "black") + ggtitle("OLG Enrichment") + scale_shape_manual(values=c(21,22))

ERC_OPC_Test <- allMods_ERCmanualOPC
ERC_OPC_Test$test_p_val <- as.numeric(ERC_OPC_Test$test_p_val)
ERC_OPC_Test$test_conflower <- as.numeric(ERC_OPC_Test$test_conflower)
ERC_OPC_Test$test_confupper <- as.numeric(ERC_OPC_Test$test_confupper)
ERC_OPC_Test$test_or <- as.numeric(ERC_OPC_Test$test_or)
...
ERC_OPC_Enrichment <- ggplot(data = ERC_OPC_Test, aes(y=reorder(test_module, test_or), x= test_or, xmin=test_conflower, xmax=test_confupper)) + geom_pointrange(aes(fill=test_module, shape = Significant), size=1.6, show.legend = FALSE) + scale_colour_manual(values = ERC_OPC_Test$test_module) + xlab("Odds Ratio (95% CI)") + ylab("Modules")+ scale_fill_manual(values = ERC_OPC_Test$test_module)+
  theme_bw() + 
  geom_vline(xintercept = 1, linetype="dashed", 
                color = "black") + ggtitle("OPC Enrichment") + scale_shape_manual(values=c(21,22))


cowplot::plot_grid(ERC_OPC_Enrichment_sig, ERC_OLG_Enrichment_sig, ncol=2 )
cowplot::plot_grid(ERC_OPC_Enrichment, ERC_OLG_Enrichment, ncol=2 )
```
